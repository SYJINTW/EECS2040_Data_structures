...
public:
    ...
    // DONE: Queue type element
    virtual T& Element() const;
    // DONE: Queue type pop
    virtual void Pop();
    // TODO: push in front of the list
...
// Queue element print the bottom one
template <class T>
T&
Bag<T>::Element() const
{
    if (IsEmpty())
        ...
        return array[(bottom + 1) % capacity];
}

// Queue Pop, pop out the bottom one
template <class T>
void
Bag<T>::Pop()
{
    ...
    array[bottom].~T(); // destructor for T
}
